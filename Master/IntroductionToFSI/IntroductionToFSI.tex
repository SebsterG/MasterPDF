...
Since there is a big difference in density between fluid and structure $\frac{\rho_f}{\rho_s} << 1$, the structural deformations will be small and the FSI problem \say{easy}. However, modeling arterial FSI deems more challenging as the density of the fluid (blood) and the structure (artery) is similar, resulting in large deformations of the arteries. Large deformations are challenging to model and require hyperelastic structure models, and energy stable numerical schemes.\newline

A scientific branch of fluid mechanics is Computational Fluid Dynamics, where computers and numerical algorithms are used to solve fluid problems. Similarly, a name used for solving FSI porblems is simply \say{FSI}. However, it should be emphasized that it is actually textit{Computational Fluid Structure Interaction} (CFSI) that will be adressed in this thesis. The same name applies to the word \say{Structure} where the actual problem to be solved belongs to the scientific branch of solid mechanics. \newline
